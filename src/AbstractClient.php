...
abstract class AbstractClient implements ClientInterface
{
    ...
    public function __construct(string $url = 'ws://127.0.0.1:9501', array $config = [])
    {
        ...
        if (!static::isSupported()) {
            throw new \RuntimeException("Your system is not supported the driver: {$driverName}, by " . static::class, -200);
        }

        $this->initConfig($config);

        $this->cliOut->write("The webSocket client power by [<info>{$driverName}</info>], remote server is <info>{$url}</info>");
    }
     ...
     */
    protected function checkResponse($header): bool
    {
        // Validate response.
        if (!preg_match('#Sec-WebSocket-Accept:\s(.*)$#mUi', $header, $matches)) {
            ...
            throw new ConnectionException("Connection to '{$this->url}' failed: Server sent invalid upgrade response header:\n$header");
        }
    ...
    protected function readLength($length)
    {
        ...
        if ($buffer === false) {
            throw new ConnectionException("Broken frame, read return 'FALSE' of stated $length bytes");
        }
    ...
    protected function write(string $data): int
    {
        ...
        if ($written < ($dataLen = \strlen($data))) {
            throw new WrittenException("Could only write $written out of $dataLen bytes.");
        }
     ...
     * @return string
     */
    protected static function bin2String($binStr): string
    {
        $string = '';
        $len = \strlen($binStr);
     ...
     * @return string
     */
    protected function encode(string $data, string $opCode = 'text', bool $masked = true, bool $final = true): string
    {
        $length = \strlen($data);
        $head = '';
        $head .= $final ? '1' : '0';
     ...
     * @return array
     */
    public function getSupportedWrappers(): array
    {
