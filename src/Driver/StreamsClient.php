...
class StreamsClient extends AbstractClient
{
    ...
    protected function doConnect($timeout = self::TIMEOUT, $flag = 0)
    {
        ...
        if ($this->socket === false) {
            throw new ConnectionException("Could not connect socket to $remote, Error: $errStr ($errNo).");
        }
    ...
    protected function readLength($length)
    {
        ...
        while (\strlen($data) < $length) {
            ...
            if ($buffer === false) {
                ...
                throw new ConnectionException('Broken frame, read ' . \strlen($data) . ' of stated '
